...
import { Input, useToast, Toast as Dialog } from '@chakra-ui/react';
...
const AuthModal = ({ show = false, onClose = () => null }) => {
  ...
  const signInWithEmail = async ({ email }:any) => {
    let toastId;
    try {
      toastId = toast({title:'Loading...'});
      setDisabled(true);
      // Perform sign in
  ...
  return (
    // <Transition appear show={show} as={Fragment}>
      <Dialog
        ...
        onClose={closeModal}
      >
        <Dialog className="fixed inset-0 bg-black opacity-75" />
            ...
            <div className="inline-block w-full my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl sm:rounded-md max-w-md relative">
              ...
              <div className="py-12">
                <div className="px-4 sm:px-12">
                  ...
                  {!showSignIn ? (
                    <Dialog className="mt-2 text-gray-500 text-base text-center">
